...
- `.gitignore` excludes `node_modules/`, `dist/`, `.DS_Store`, and root-level `*.jpg`, `*.jpeg`, `*.mov` source media
- Raw source media that used to live at the repository root have been removed as repo-hygiene cleanup; originals are recoverable from git history — see [`docs/ASSETS.md`](docs/ASSETS.md)
